    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if images:
                ...
                # 测试页面位置固定功能
                test_page_position_features(base_url, version_name)
            else:
...
def test_page_position_features(base_url, version_name):
    """测试页面位置固定功能"""
    print(f"\n🎯 测试 {version_name} 页面位置固定功能")
    print("-" * 40)
    
    print(f"💡 手动测试步骤:")
    print(f"1. 访问 {base_url}")
    print(f"2. 滚动页面到任意位置")
    print(f"3. 测试以下按钮的页面位置固定功能:")
    print(f"   📋 加载图像按钮:")
    print(f"      - 点击后页面应该滚动到顶部")
    print(f"      - 页面位置应该固定在 (0, 0)")
    print(f"   ⬅️ 上一张按钮:")
    print(f"      - 点击后页面应该滚动到顶部")
    print(f"      - 页面位置应该固定在 (0, 0)")
    print(f"   ➡️ 下一张按钮:")
    ...
    print(f"      - 页面位置应该固定在 (0, 0)")
    print(f"4. 验证功能效果:")
    print(f"   - 每次点击按钮后页面都回到顶部")
    print(f"   - 没有页面跳动或滚动异常")
    print(f"   - 用户体验流畅自然")

def create_page_position_fix_instructions():
    """创建页面位置固定功能说明"""
    instructions = """
# 页面位置固定功能说明
...
"""
    ...
    with open("datasets/PAGE_POSITION_FIX_INSTRUCTIONS.md", "w", encoding="utf-8") as f:
        f.write(instructions)
    
    print("📄 已创建页面位置固定功能说明文档: datasets/PAGE_POSITION_FIX_INSTRUCTIONS.md")
...
def main():
    ...
    # 测试页面位置固定功能
    test_page_position_fix()
    
    # 创建功能说明
    create_page_position_fix_instructions()
    
    print("\n🎉 测试完成！")
    print("\n💡 使用建议:")
    print("1. 访问标注工具测试页面位置固定功能")
    print("2. 滚动页面到任意位置")
    print("3. 点击各个按钮验证页面是否回到顶部")
    print("4. 确认操作体验流畅自然")
